...
export default function App() {
  ...
  return (
    <div className="min-h-screen relative">
            ...
            {view === 'expenses' && (
              <>
                <div className="flex items-center justify-between">
                  <h1 className="text-lg font-bold text-slate-800 dark:text-white">All Expenses</h1>
                </div>
                ...
                <div className="card">
                  <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-white/5">
                    <div>
                      <h2 className="text-sm font-semibold text-slate-800 dark:text-white">
                        {expLoading ? 'Loading...' : `${expenses.length} expense${expenses.length !== 1 ? 's' : ''}`}
                      </h2>
                    </div>
                    <button className="btn-primary text-xs py-2 px-3" onClick={() => setShowForm(v => !v)}>
                      <PlusCircle className="h-3.5 w-3.5" /> Add
                    </button>
                  </div>
                  {showForm && (
                    <div className="px-5 pb-5 pt-4 border-b border-slate-100 dark:border-white/5">
                  ...
                  {expError
                    ...
                    : <ExpenseTimeline expenses={expenses} loading={expLoading} onEdit={setEditingExpense} onDeleted={handleDeleted} />
                  }
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="card overflow-hidden">
                  ...
                  >
                    ...
                    {showForm ? <ChevronUp className="h-4 w-4 text-slate-400" /> : <ChevronDown className="h-4 w-4 text-slate-400" />}
                  </button>
                  {showForm && (
                ...
                <div className="card">
                  <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-white/5">
                    <div>
                      <h2 className="text-sm font-semibold text-slate-800 dark:text-white">
                        {expLoading ? 'Loading...' : `Recent Expenses (${expenses.length})`}
                      </h2>
      ...
      {editingExpense && (
        <EditModal expense={editingExpense} onSaved={handleEditSaved} onClose={handleCloseEdit} />
      )}
    </div>
  )
}
